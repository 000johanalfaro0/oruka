...
description: Optional real browser automation via blop-browser CLI. Use ONLY when explicitly requested by the user, or when a task specifically requires UI rendering, visual testing, clicks, forms, or screenshots. DO NOT use for simple web search or reading text.
license: MIT
...
## Shared session convention (this machine)

For the user's own personal accounts (email, social apps, etc.) always use
the same fixed session name: `--session personal`. Do not invent a new
session name per task or per CLI. Every agent on this machine — Claude,
Codex, Gemini/Agy, Opencode, and anything Oruka launches — must reuse this
one `personal` session so that a login done once (or via human takeover)
stays valid for all of them. Only use a different, disposable session name
when the user explicitly asks for an isolated or throwaway test that should
not touch the shared logins.

**Never run `destroy` (or `data delete`) on the `personal` session.** That
permanently wipes its profile and every saved login in it. `close` is fine
(it disconnects the CLI but keeps the profile on disk). If `personal` truly
needs to be wiped, that is the user's call, not an agent's — ask first.
